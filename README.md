...
- **Hardware/sensor_and_control.ino**: 차량을 구성하는 코드. 적외선 리모콘으로 차량을 움직이고, 앞차량과의 거리를 감지하여, 메인 컴퓨터인 라즈베리파이로 플래그를 전송하는 거리 감지 정보 플래그를 전송함.
...
- **Hardware/ledwarning.ino**: 시리얼 통신으로 수신한 플래그(0, 1)에 따라서, 경고등을 켜거나 끄는 제어 로직을 담당하는 아두이노.
